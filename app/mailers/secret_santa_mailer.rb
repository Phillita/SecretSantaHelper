...
class SecretSantaMailer < ApplicationMailer
  ...
  def participant(participant_id, receiver_name, filepath = nil)
    ...
    options = liquid_options(participant.participantable, participant, receiver_name)
    email_subject = parse_liquid(participant.participantable.email_subject, options)
    email_content = parse_liquid(participant.participantable.email_content, options)
